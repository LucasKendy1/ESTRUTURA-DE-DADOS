//Execute o programa do Exemplo 6 e veja o que acontece quando o 
//usuário tenta preencher uma região com a mesma cor que ela já tem. 
//Altere a função colorir() para corrigir o problema.
...
void colorir(int I[dim][dim], int i, int j, int n) {
  ...
  int a = I[i][j];//a = [4][4]
	if(cor(i,j)!= n){
		...
		printf("A coordenada ja possui a mesma cor!\n");
  	}
  destroif(&F);
}
